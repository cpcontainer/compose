...
NOTE:
This version maps to the array returned by these options and puts to status/starlink
dish_grpc_text_CP2.py status alert_detail usage -t 60
"""
...
def loop_body(opts, gstate, print_file, shutdown=False):
    ...
    if opts.verbose:
        ...
        if csv_data:
            ...
            try:
                # This maps to the array returned by these options:
                # dish_grpc_text_CP3.py status alert_detail usage -t 60
                fields = ['timestamp', 'id', 'hardware_version',
                          'software_version', 'state', 'uptime', 'snr', 'seconds_to_first_nonempty_slot',
                          'pop_ping_drop_rate', 'downlink_throughput_bps', 'uplink_throughput_bps', 'pop_ping_latency_ms',
                          'unknown1', 'Alerts bit field', 'currently_obstructed',
                          'seconds_obstructed', 'obstruction_duration', 'obstruction_interval', 'direction_azimuth',
                          'direction_elevation', 'is_snr_above_noise_floor', 'alert_motors_stuck',
                          'alert_thermal_throttle', 'alert_thermal_shutdown', 'alert_mast_not_near_vertical',
                          'alert_unexpected_location', 'alert_slow_ethernet_speeds', 'alert_roaming',
                          'alert_install_pending', 'alert_is_heating', 'alert_power_supply_thermal_throttle',
                          'alert_is_power_save_idle', 'alert_moving_while_not_mobile',
                          'alert_moving_too_fast_for_policy', 'alert_dbf_telem_stale', 'unknown2', 'unknown3',
                          'unknown4', 'unknown5', 'Parsed samples', 'Sample counter', 'Bytes downloaded', 'Bytes uploaded']
                body = {}
                for i, field in enumerate(csv_data):
                    body[fields[i]] = field
                body = {k: v for k, v in body.items() if "unknown" not in k}
                cp.put("status/starlink", body)
                cp.log(body)
                connected = ['🔴', '🟢'][body.get("state", 'DISH_UNREACHABLE') == 'CONNECTED']
                obstructed = 'Obstructed' if body.get("currently_obstructed", 'False') == 'True' else 'Unobstructed'
                down = f'{float(body.get("downlink_throughput_bps", 0)) / 1000 / 1000:.2f}Mbps'
                up = f'{float(body.get("uplink_throughput_bps", 0)) / 1000 / 1000:.2f}Mbps'
                latency = f'{float(body.get("pop_ping_latency_ms", 0)):.0f}ms'
                loss = f'{float(body.get("pop_ping_drop_rate", 0)):.0f}%'
                asset = f'📡 Connected: {connected} | Down: {down} | Up: {up} | Latency: {latency} | Loss: {loss} | 🚧: {obstructed}'
                cp.put("config/system/asset_id", asset)
            except Exception as e:
                cp.put("config/system/asset_id", "📡 Error parsing Starlink data")
                cp.logger.exception(e)
        else:
            cp.put("config/system/asset_id", "📡 Starlink not connected")
    return rc
...
if __name__ == "__main__":
    cp = CSClient('starlink-grpcs-tools')
    main()
